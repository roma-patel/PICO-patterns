...
set_path = '/nlp/data/romap/naacl-pattern/crf/joint/2/'
...
def word_features(sent, i, indwords, aslog, pos_patterns):
    ...
    features = [
        ...
        'word[-3:]=' + word[-3:],
        'word[-2:]=' + word[-2:],
        'part.ind=%s' % (word_lower in aslog[0]),
        'int.ind=%s' % (word_lower in aslog[1]),
        'out.ind=%s' % (word_lower in aslog[2]),
    ...
    if i > 0:
        ...
        features.append('-1:part.ind=%s' % (bigram_prev in aslog[0]))
        features.append('-1:int.ind=%s' % (bigram_prev in aslog[1]))
        features.append('-1:out.ind=%s' % (bigram_prev in aslog[2]))
    ...
    if i < len(sent) - 1:
        ...
        features.append('+1:part.ind=%s' % (bigram_next in aslog[0]))
        features.append('+1:int.ind=%s' % (bigram_next in aslog[1]))
        features.append('+1:out.ind=%s' % (bigram_next in aslog[2]))
