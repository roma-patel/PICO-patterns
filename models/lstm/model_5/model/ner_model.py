...
class NERModel(BaseModel):
    ...
    def add_placeholders(self):
        ...
        # shape = (batch size, max length of sentence, 3)
        self.ind_ids = tf.placeholder(tf.int32, shape=[None, None],
                        name="ind_ids")
        ...
        # shape = (batch_size, 1
        self.ind_lengths = tf.placeholder(tf.int32, shape=[None],
                        name="ind_lengths")
    ...
    def get_feed_dict(self, words, labels=None, lr=None, dropout=None):
        ...
        if self.config.use_chars:
            char_ids, word_ids, ind_ids = zip(*words)
            ...
            ind_ids, ind_lengths = pad_sequences(ind_ids, 0)
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("words"):
            ...
            if self.config.indicator_embeddings is not None:
                _ind_embeddings = tf.Variable(
                        self.config.indicator_embeddings,
                        name="_ind_embeddings",
                        dtype=tf.float32,
                        trainable=self.config.train_embeddings)

                ind_embeddings = tf.nn.embedding_lookup(_ind_embeddings,
                    self.ind_ids, name="ind_embeddings")
                s = tf.shape(ind_embeddings)

                # shape = (batch size, max sentence length, char hidden size)
                word_embeddings = tf.concat([word_embeddings, ind_embeddings], axis=-1)
